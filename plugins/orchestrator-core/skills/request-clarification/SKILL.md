...
description: Detection and resolution of ambiguities through targeted questioning, producing CONTEXT REPORTS with structured requirements. Load when requests are vague, scope is undefined, multiple interpretations exist, or critical information is missing before execution.
triggers:
  - clarify
  - ambiguous
  - unclear
...
The request clarification skill enables the orchestrator to identify ambiguities, missing information, and implicit assumptions in user requests, then formulate precise questions to resolve these uncertainties before proceeding with agent deployment or execution. This skill prevents wasted effort from misunderstood requirements and ensures alignment between user intent and orchestrator actions.
